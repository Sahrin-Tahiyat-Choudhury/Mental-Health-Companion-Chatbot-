...
# Load .env variables locally (not needed on Streamlit Cloud if using st.secrets)
load_dotenv()
...
# Sidebar for nickname and info
with st.sidebar:
    st.header("Settings")
    companion_name = st.text_input("AI Companion Nickname:", value="CalmMate")
    st.markdown("*Mood Overview Tab:* Check your mood history here.")

# Tabs for conversation and mood
tabs = st.tabs(["💬 Chat", "📊 Mood Overview"])
chat_tab, mood_tab = tabs
...
with chat_tab:
    ...
    # Chat input form (input clears automatically)
    ...
    for chat in st.session_state.history[::-1]:  # newest at bottom
        user_msg = f"*You:* {chat['user']}"
        ai_msg = f"{companion_name}:** {chat['reply']}"
        mood_emoji = {
            "Happy": "😊",
            "Sad": "😢",
            "Stressed": "😟",
